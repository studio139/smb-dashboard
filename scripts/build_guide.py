...
def build():
    ...
    bullet(doc, [("הייצואים ", False), ("מצטברים", False),
                 (" — מניחים את הקובץ המלא, לא חותכים תאריכים; המנוע מסנן בעצמו לחודש או לרבעון.", False)])
    bullet(doc, [("טווחים: ", False), ("לקוחות (לידים)", False),
                 (" — מתחילת 2025 או אף קודם (כמה שיש), כ-lookback ללידים; ", False),
                 ("פרויקטים · הכנסות · שתפים · משימות", False),
                 (" — מ-1.1.2026. כולם עד סוף החודש הנוכחי.", False)])
    bullet(doc, [("לידים מ-2025 (או קודם) הם ", False), ("lookback בלבד", False),
                 (" — לא נספרים כלידי 2026. היסטוריה רחבה יותר מעלה את השיוך ועשויה להזיז מעט את שיעור "
                  "ההמרה המיוצב — זה תקין וצפוי, לא שגיאה.", False)])
    ...
    # 2
    heading(doc, "2. הרצת דוח — מה מפיקים בכל חודש")
    bullet(doc, [("פותחים טרמינל בתיקייה ומריצים את ", False), ("החודש", False),
                 (" — חודש שסוגר רבעון מפיק אוטומטית גם את הרבעוני, ודצמבר גם את הרבעוני והשנתי:", False)])
    code_line(doc, r"C:\Python314\python.exe scripts\run.py 2026-05      (חודשי בלבד)")
    code_line(doc, r"C:\Python314\python.exe scripts\run.py 2026-03      (חודשי + רבעוני Q1 — אוטומטי)")
    code_line(doc, r"C:\Python314\python.exe scripts\run.py 2026-12      (חודשי + רבעוני Q4 + שנתי)")
    code_line(doc, r"C:\Python314\python.exe scripts\run.py 2026-Q1      (רק הרבעוני, לפי דרישה)")
    bullet(doc, [("הכלל: מרץ/יוני/ספטמבר מוסיפים רבעוני; דצמבר מוסיף רבעוני ושנתי; שאר החודשים — חודשי בלבד. "
                  "אין מה לזכור — פשוט מריצים את החודש.", False)])
    bullet(doc, [("הפלט מאורגן שנה ← סוג ← תקופה תחת ", False), ("outputs\\", True),
                 (" — למשל ", False), ("outputs\\2026\\חודשי\\2026-04\\", True),
                 (": קובץ ", False), (".xlsx", True), (" ותצוגת ", False), (".html", True),
                 (" בלבד (ללא PDF). אותו מבנה גם ב-Drive.", False)])
    ...
    bullet(doc, [("בכל חודש שסוגר רבעון נוצרת אוטומטית תבנית ", False), ("Word", True),
                 (" ריקה בתוך תיקיית הפלט של אותו רבעון (תחת ", False), ("outputs", True),
                 (", בענף הרבעוני) — למשל הקובץ ", False), ("2026-Q1_יעדים.docx", True),
                 (" בתיקיית הרבעון ", False), ("2026-Q1", True),
                 (". התבנית נושאת את יעדי הרבעון הבא.", False)])
    bullet(doc, [("ממלאים את ", False), ("טבלת היעדים", False),
                 (" — מספר לכל מדד (פניות · עסקאות · המרה · מחזור · רווח · תקרת הוצאות). "
                  "שמות המדדים מתחברים אוטומטית לדוח. התובנות והיוזמות הן טקסט חופשי — לא נשלף מהן מספר.", False)])
    bullet(doc, [("ממלאים את ", False), ("העותק המקומי", False),
                 (" (הוא זה שנקרא קדימה). המסמך ", False), ("לעולם לא נדרס", False),
                 (" — תיקיית הפלט נבנית מחדש בכל ריצה, אבל וורד היעדים נשמר, ומגובה ל-", False),
                 ("Drive", True), (" וגם ל-", False), ("GitHub", True), (".", False)])
    bullet(doc, [("כל דוח קורא את היעדים ממסמך ", False), ("הרבעון הקודם", False),
                 (" — כולל ", False), ("חציית שנה", False), (": ינואר–מרץ קוראים את ", False),
                 ("Q4", True), (" של השנה הקודמת. מסמך חסר או עדיין ריק ← הדוח יוצא בלי יעדים (אין שגיאה).", False)])
    ...
    bullet(doc, [("העלאה ל-", False), ("Drive", True),
                 ...
                 (") עולים ל-Drive המשותף, תחת «ניתוחים וישיבות הנהלה» לפי אותו מבנה כמו מקומית — "
                  "«דשבורדים» ← שנה ← סוג ← תקופה. מתחבר עם החשבון ", False), ("studio@smb-arch.com", True), (".", False)])
